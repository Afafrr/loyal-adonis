...
export function CompanyCard({ account }: { account: LoyaltyAccount }) {
  ...
  return (
    <article className='min-w-0 rounded-3xl border border-line-subtle bg-panel p-4 shadow-[0_14px_36px_rgba(32,42,37,0.035)] sm:rounded-[18px] sm:px-10 sm:py-8 lg:px-11 lg:py-9'>
        ...
        <div className='mt-3 border-t border-line-faint pt-4'>
          ...
          <ul className='mt-1'>
            {account.locations.map((location) => (
              <li
                className='flex items-baseline justify-between gap-4 text-sm font-bold lg:text-[13px]'
                key={location.id}
              >

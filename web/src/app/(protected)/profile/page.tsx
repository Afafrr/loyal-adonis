...
export default async function ProfilePage() {
  ...
  return (
    <section className='mx-auto max-w-2xl px-4 pb-12 pt-2 min-[380px]:px-5 sm:px-12 sm:pb-16 sm:pt-6'>
      <div className='flex items-center justify-between gap-4'>
        <PageTitle>Profile</PageTitle>
        <span className='rounded-full border border-line-subtle bg-panel px-4 py-2 text-xs font-bold text-foreground-secondary shadow-control sm:text-sm'>
          Edit
        </span>
      </div>

      <div className='mt-7 flex flex-col items-center sm:mt-9'>
        <div className='grid size-24 place-items-center rounded-full bg-avatar text-4xl font-black text-avatar-foreground shadow-avatar sm:size-28 sm:text-5xl'>
          {userInitial(profile)}
        </div>
        <h2 className='mt-3 max-w-full truncate text-2xl font-extrabold tracking-[-0.035em] sm:text-3xl'>{displayName}</h2>
        <p className='mt-1 text-xs font-semibold text-foreground-muted sm:text-base'>Loyal Nest member since {memberSince}</p>
      </div>

      <div className='mt-6 flex items-end justify-between rounded-[26px] bg-brand px-8 py-5 text-white shadow-brand sm:mt-8 sm:px-12 sm:py-6'>
        <div>
      ...
      <div className='overflow-hidden rounded-[24px] border border-line-subtle bg-panel shadow-card'>
